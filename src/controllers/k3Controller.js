...
const rosesPlus = async (auth, money) => {
     ...
     if (money >= 10000) {
         if (f1.length > 0) {
             ...
             await connection.query('UPDATE users SET money = money + ?, roses_f1 = roses_f1 + ?, roses_f = roses_f + ?, roses_today = roses_today + ? WHERE phone = ? ', [rosesF1, rosesF1, rosesF1, infoF1.phone ]);
...
const validateBet = async(join, list_join, x, money, game) => {
     ...
     const checkMoney = ['1000', '100000', '100000', '1000000'].includes(money);
...
const betK3 = async(req, res) => {
     try {
         ...
         if (gameJoin == 1) {
             ...
             let motDuyNhat = listJoin.split('@')[1]; // Choose a pair of unique numbers
             if (twoSame.length > 0) {
             ...
             let threeSame = listJoin.split('@')[1].length; // Choose 3 same numbers
             total = money * xvalue * countBaDuyNhat + (threeSame * money * xvalue);
         } else if (gameJoin == 4) {
             let threeNumberUnlike = listJoin.split('@')[0]; // Choose 3 unique numbers
             let twoLienTiep = listJoin.split('@')[1]; // Select 3 consecutive numbers
...
async function funHanding(game) {
     ...
     for (let i = 0; i < totalN; i++) {
         ...
         // handle the game with 2 numbers that match
         ...
         // handle the game with 3 duplicate numbers
        ...
        // handle the game with 3 different numbers
       ...
       async function plusMoney(game) {
            ...
            for (let i = 0; i < order.length; i++) {
                ...
                if (orders.typeGame == "total") {
                    ...
                    n_duoc += price * get;
                    await connection.execute('UPDATE `result_k3` SET `get` = ?, `status` = 1 WHERE `id` = ?', [name_duoc, id]);
                    const sql = 'UPDATE `users` SET `money` = `money` + ? WHERE `phone` = ? ';
                    await connection.execute(sql, [name_duoc, phone]);
                }
                n_duoc = 0;
                    ...
                    for (let i = 0; i < arr1.length; i++) {
                        if (arr1[i] != "") {
                            ...
                            if (check1) {
                                ...
                                nha_duoc += total * 207.36 - orders.fee;
                            }
                        }
                    }
                    if (arr2 != "") {
                        ...
                        nha_duoc += total * 34.56 - orders.fee;
                    }
                    await connection.execute('UPDATE `result_k3` SET `get` = ?, `status` = 1 WHERE `id` = ?', [name_duoc, id]);
                    const sql = 'UPDATE `users` SET `money` = `money` + ? WHERE `phone` = ? ';
                    await connection.execute(sql, [name_duoc, phone]);
                        ...
                        if (arr1[i] != "") {
                            ...
                            if (!check1) {
                                ...
                                if (arr2 == "u") {
                                    ...
                                    nha_duoc += (total - orders.fee) * 8.64;
                                }
                            }
                        }
                    }
                    ...
                    for (let i = 0; i < arr3.length; i++) {
                        if (arr1[i] != "") {
                            ...
                            if (!check1) {
                                ...
                                nha_duoc += total * 6.91 - orders.fee;
                            }
                        }
                    }
                    await connection.execute('UPDATE `result_k3` SET `get` = ?, `status` = 1 WHERE `id` = ?', [name_duoc, id]);
