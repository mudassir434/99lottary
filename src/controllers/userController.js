     ...
     if(data) {
         if(data == 1) {
             ...
             if (check >= data && point_list.total1 != 0) {
                 ...
                 return res.status(200).json({
                     message: `You just received ${point_list.total1}.00 TRX`,
                     status: true,
                     timeStamp: timeNow,
                 });
         ...
         if(data == 2) {
             ...
             if (check >= get && point_list.total2 != 0) {
                 ...
                 return res.status(200).json({
                     message: `You just received ${point_list.total2}.00 TRX`,
                     status: true,
                     timeStamp: timeNow,
                 });
         ...
         if(data == 3) {
             ...
             if (check >= get && point_list.total3 != 0) {
                 ...
                 return res.status(200).json({
                     message: `You just received ${point_list.total3}.00 TRX`,
                     status: true,
                     timeStamp: timeNow,
                 });
        ...
        if(data == 4) {
            ...
            if (check >= get && point_list.total4 != 0) {
                ...
                return res.status(200).json({
                    message: `You just received ${point_list.total4}.00 TRX`,
                    status: true,
                    timeStamp: timeNow,
                });
        ...
        if(data == 5) {
            ...
            if (check >= get && point_list.total5 != 0) {
                ...
                return res.status(200).json({
                    message: `You just received ${point_list.total5}.00 TRX`,
                    status: true,
                    timeStamp: timeNow,
                });
        ...
        if(data == 6) {
            ...
            if (check >= get && point_list.total6 != 0) {
                ...
                return res.status(200).json({
                    message: `You just received ${point_list.total6}.00 TRX`,
                    status: true,
                    timeStamp: timeNow,
                });
        ...
        if(data == 7) {
            ...
            if (check >= get && point_list.total7 != 0) {
                ...
                return res.status(200).json({
                    message: `You just received ${point_list.total7}.00 TRX`,
                    status: true,
                    timeStamp: timeNow,
                });
...
const tronupdate = async(req, res) => {
    let auth = req.cookies.auth;
    let money = req.body.money;
    if(!auth) {
        return res.status(200).json({
    ...
    const [rechargelen] = await connection.query('SELECT * FROM recharge WHERE phone = ? AND status = ? ', [userInfo.phone, 0]);
    if (rechargelen.length == 0) {
        const date = new Date();
        let id_time = date.getUTCFullYear() + '' + date.getUTCMonth() + 1 + '' + date.getUTCDate();
        let id_order = Math.floor(Math.random() * (99999999999999 - 10000000000000 + 1) ) + 10000000000000;
        let time = new Date().getTime();
    ...
    url = ?,
    time = ?`;
    await connection.execute(sql, [client_transaction_id, '0', userInfo.phone, 5, 'bank', 0, checkTime, '0', time]);
    }
    const [recharge] = await connection.query('SELECT * FROM recharge WHERE phone = ? AND status = ? ', [userInfo.phone, 0]);
    const [bank_recharge] = await connection.query('SELECT * FROM bank_recharge ');
    if(recharge[0].address==0){
        const data = {
            price_amount: recharge[0].money,
            price_currency: 'trx',
            pay_currency: 'trx',
            ipn_callback_url: 'https://trx.up.railway.app/verifyorder',
            order_id: recharge[0].id_order,
            order_description: 'Pay For Order'
        };
        
        const headers = {
            'Content-Type': 'application/json',
            'x-api-key': bank_recharge[0].stk
        };
        
        axios.post('https://api.nowpayments.io/v1/payment', data, { headers })
            .then(response => {
                const response_data = response.data;
        
                const payment_id = response_data.payment_id;
                const address = response_data.pay_address;
                const pay_amount = response_data.pay_amount;
                const price_amount = response_data.price_amount;
                const payment_status = response_data.payment_status;
                const price_currency = response_data.price_currency;
        
                const pay_currency = response_data.pay_currency;
                const created_at = response_data.created_at;
                connection.query(
                    'UPDATE recharge SET address = ?, money = ?, transaction_id = ? WHERE phone = ? AND status = ?',[address, price_amount, payment_id, userInfo.phone, 0]);                
                if (recharge.length !== 0) {
                    return res.status(200).json({
                        message: 'Received success',
                        datas: recharge[0],
                        infoBank: bank_recharge,
                        status: true,
                        address: address, // Include the address here
                        amount: price_amount,
                        timeStamp: timeNow,
                    });
                }else {
                ...
                // Insert into database or perform other actions as needed
            }).catch(error => {
        ...
        if (recharge.length !== 0) {
            return res.status(200).json({
                ...
                infoBank: bank_recharge,
                status: true,
                address: recharge[0].address, // Include the address here
                amount: recharge[0].money,
                timeStamp: timeNow,
            });
        }else {
            return res.status(200).json({
...
const tronverify = async(req, res) => {
    console.log("verify");
    const [rows] = await connection.execute('SELECT * FROM recharge WHERE status = ?', [0]);

        // Loop through the retrieved rows
        for (const row of rows) {
            const payment_id = row.transaction_id;
            ...
            const { payment_status, outcome_amount } = response.data;
            console.log(`Payment ID: ${payment_id}, Status: ${payment_status}, Amount: ${outcome_amount}`);
            // Process the payment_status and price_amount as needed
          if(payment_status=="partially_paid"||payment_status=="finished"){
            await connection.query(`UPDATE recharge SET status = 1 WHERE transaction_id = ?`, [payment_id]);
            const [info] = await connection.query(`SELECT * FROM recharge WHERE transaction_id = ?`, [payment_id]);
            await connection.query('UPDATE users SET money = money + ?, total_money = total_money + ? WHERE phone = ? ', [outcome_amount, outcome_amount, info[0].phone]);
          
          }else if(payment_status=="failed"||payment_status=="refunded"||payment_status=="expired"){
            await connection.query(`UPDATE recharge SET status = 2 WHERE transaction_id = ?`, [payment_id]);
